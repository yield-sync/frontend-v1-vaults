	...
	<VRow>
		<VCol cols="12" md="8">
			<fieldset class="border-0">
				<input
					v-model="this.$store.state.view.v1Vault.transferRequestB.for"
					type="radio"
					value="Ether"
					id="Ether"
				/>
		...
		<VCol cols="12" md="4">
			<select
				v-if="this.$store.state.view.v1Vault.transferRequestB.for != 'Ether'"
				v-model="this.$store.state.view.v1Vault.transferRequestB.token"
				class="w-100 px-4 py-2 bg-light border rounded"
				@change="this.$store.state.view.v1Vault.transferRequestB.for = 'ERC 20'"
			>
		...
		<VCol cols="12">
			<!-- TO -->
			<VTextField
				v-model="this.$store.state.view.v1Vault.transferRequestB.to"
				type="text"
				label="To Address"
				variant="outlined"
				hide-details
		...
		<VCol cols="6">
			<!-- TOKEN ADDRESS -->
			<VTextField
				:disabled="this.$store.state.view.v1Vault.transferRequestB.for == 'Ether'"
				v-model="this.$store.state.view.v1Vault.transferRequestB.token"
				type="text"
				label="Token Address"
				variant="outlined"
		...
		<VCol cols="2">
			<!-- TOKEN ID -->
			<VTextField
				:disabled="this.$store.state.view.v1Vault.transferRequestB.for != 'ERC 721'"
				v-model="this.$store.state.view.v1Vault.transferRequestB.tokenId"
				type="number"
				label="Token Id"
				variant="outlined"
				hide-details
		...
		<VCol cols="4">
			...
			<VTextField
				:disabled="this.$store.state.view.v1Vault.transferRequestB.for == 'ERC 721'"
				v-model="this.$store.state.view.v1Vault.transferRequestB.amount"
				type="number"
				label="Amount"
				variant="outlined"
				hide-details
		...
		<VCol cols="4">
			<!-- Vote Close Time -->
			<VTextField
				v-model="this.$store.state.view.v1Vault.transferRequestB.voteCloseTimestamp"
				type="number"
		...
		<VCol cols="8">
			<h5 class="text-dark">
				Minutes: {{ this.$store.state.view.v1Vault.transferRequestB.voteCloseTimestamp / 60 }} -
				Hours: {{ this.$store.state.view.v1Vault.transferRequestB.voteCloseTimestamp / (60 * 60) }}
				<br/>
				Current Block Timestamp + Vote Close Time = Submittable Block Timestamp
				<br/>
				{{ this.currentBlockTimestamp }} + {{
					this.$store.state.view.v1Vault.transferRequestB.voteCloseTimestamp
				}} = {{
		...
		methods: {
			...
			async createWR(): Promise<void>
			{
				...
				transferRequestProtocol.methods.yieldSyncV1Vault_transferRequestId_transferRequestCreate(
					this.vaultAddress,
					this.$store.state.view.v1Vault.transferRequestB.for == "ERC 20" ? true : false,
					this.$store.state.view.v1Vault.transferRequestB.for == "ERC 721" ? true : false,
					this.$store.state.view.v1Vault.transferRequestB.to,
					(
						this.$store.state.view.v1Vault.transferRequestB.token
					) ? this.$store.state.view.v1Vault.transferRequestB.token : this.ZERO_ADDRESS,
					(
						this.$store.state.view.v1Vault.transferRequestB.for == "ERC 721"
					) ? BigInt(10 ** 18) : BigInt(this.$store.state.view.v1Vault.transferRequestB.amount * 10 ** 18),
					this.$store.state.view.v1Vault.transferRequestB.tokenId,
