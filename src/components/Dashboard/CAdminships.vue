	...
	<VCard class="mb-4 rounded-xl bg-light-frost elevation-0">
		<VCardText class="px-6 py-6">
			...
			<div v-if="!this.loading">
				<RouterLink
					v-for="(v, i) in this.$store.state.adminshipYieldSyncV1VaultVaults"
					:key="i"
			...
			<div
				v-if="!this.loading && this.$store.state.adminshipYieldSyncV1VaultVaults.length == 0"
				class="text-center"
			>
...
<script lang="ts">
	...
	export default defineComponent({
		...
		async created(): Promise<void>
		{
			...
			await this.$store.dispatch("generateVaultAdminships");
